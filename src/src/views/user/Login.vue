  ...
  <div class="main">
    <a-form id="formLogin" class="user-layout-login" ref="formLogin" :form="form" @submit="handleSubmit">
      ...
      <div class="user-login-other">
        ...
        <span @click="forGetPwd(1)" class="forge-password" style="float: right; color: #1890ff; cursor: pointer">{{
          $t('user.login.forgot-password')
        }}</span>
        <a-modal v-model="forGetPwdModal.visible" :title="$t('modal.notice.title')">
          ...
          <p>{{ $t('user.forgetpassword.modal.content') }}</p>
        </a-modal>
    ...
    handleSubmit(e) {
      ...
      validateFields(validateFieldsKey, { force: true }, (err, values) => {
        if (!err) {
          ...
          loginParams.userPwd = values.password
